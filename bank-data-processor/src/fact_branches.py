...
cursor.execute('''
CREATE TABLE IF NOT EXISTS fact_branches (
    id TEXT PRIMARY KEY,
    branch_name TEXT,
    primary_type TEXT,
    bank_name TEXT,
...
# Load the JSON data
with open('/home/aizeria/Documents/Customer-Reviews-of-Bank-Agencies-in-Morocco/data-of-banks-2.json', 'r') as file:
    data = json.load(file)
...
for bank in data:
    try:
        ...
        cursor.execute('DELETE FROM fact_branches WHERE id = %s', (id,))

        cursor.execute('''
        INSERT INTO fact_branches (id, branch_name, primary_type, bank_name, region, rating, user_rating_count,
        ...
        ''', (id, f"{display_name} {address}", primary_type, bank_name, region, rating, user_rating_count, *review_data, *sentiment_topic_data))
